...
get.test.statistics <- function(sdata, format = "DNAbin", geneName = "empirical", phymlPath, model = "GTR+G"){
	...
	phymlres <- runPhyML(data, temp_name = geneName, phymlPath = phymlPath, model = model)
	
	# Get test statistics.
	chisq <- getchisqs(data)
	homoplasy <- CI(phymlres$outputTree, data)

	# Return test statistics, tree, and parameter estimates.
	
	results <- list(multinomial.lik = phymlres$uncLikelihood, chisq.stat = chisq, homoplasy = homoplasy, mean.branch.sup = phymlres$meanNodeSupport, CI.branch.sup = phymlres$nodeSupport95, delta = phymlres$delta, tree.length = phymlres$treeLength, outputTree = phymlres$outputTree)
